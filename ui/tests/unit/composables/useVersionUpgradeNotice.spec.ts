...
function setup(options: {configs?: Record<string, unknown>, userId?: string} = {}) {
    useMiscStore().configs = options.configs as never
    if (options.userId) {
        // Mirrors the EE auth store override, whose `user` carries a real account id.
        useAuthStore().user = {id: options.userId} as never
    }
...
describe("useVersionUpgradeNotice", () => {
    ...
    it("still shows the notice to a different user on the same browser", () => {
        setup({configs: {uuid: "i1", versionUpgrade: UPGRADE}, userId: "user-a"}).dismiss()

        setActivePinia(createPinia())
        const other = setup({configs: {uuid: "i1", versionUpgrade: UPGRADE}, userId: "user-b"})
        expect(other.visible.value).toBe(true)

        // and dismissing it there leaves the first user's dismissal intact
        other.dismiss()
        setActivePinia(createPinia())
        expect(setup({configs: {uuid: "i1", versionUpgrade: UPGRADE}, userId: "user-a"}).visible.value).toBe(false)
    })
